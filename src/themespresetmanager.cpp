...
theme_presets_manager::theme_presets_manager(abstract_themes_source *source) {
  themes_source = source;
}

std::vector<std::string>
theme_presets_manager::get_possible_preload_theme_alias() {
...
bool base_local_fs_folder_source::load_themes_db() {
  ...
  for (const auto &entry :
       std::filesystem::directory_iterator(path_to_folder)) {
...
system_git_repo_source::~system_git_repo_source() { git_libgit2_shutdown(); }
...
bool system_git_repo_source::pull_changes(git_repository *target,
                                          bool use_base_branch) {
  if (!fetch_repo()) {
  ...
  if (!use_base_branch) { branch_name = get_head_branch_name_for_repo(target); }

  if (git_reference_lookup(
          &mainBranch, target,
          (std::string("refs/remotes/origin/") + branch_name).c_str()) != 0) {
    std::cout << "system_git_repo_source::error while git lookup (main): "
              << git_error_last()->message << std::endl;
    return false;
  }
  git_annotated_commit *lastMainBranchCommit;
  if (git_annotated_commit_from_ref(&lastMainBranchCommit, target,
                                    mainBranch) != 0) {
...
bool system_git_repo_source::pull_changes() { return pull_changes(repo); }
...
bool system_git_repo_source::is_getted_initilaized_repo_submodule(
    git_submodule *submodule, git_repository **repo_result) {
...
bool system_git_repo_source::fetch_repo(git_repository *target) {
  ...
  if (git_remote_lookup(&remote, target, "origin") != 0) {
    show_last_git_error(
        "system_git_repo_source::error while git remote lookup (origin): ");
  ...
  if (git_remote_fetch(remote, NULL, NULL, "fetch") != 0) {
    git_remote_free(remote);
    show_last_git_error(
        "system_git_repo_source::error while git remote fetch (origin): ");
    return false;
  }
  git_remote_free(remote);
  return true;
...
bool system_git_repo_source::fetch_repo() { return fetch_repo(repo); }

std::string system_git_repo_source::get_head_branch_name_for_repo(
...
bool system_git_repo_source::is_submodule_of_repo(
    std::string path, git_submodule **result_submodule) {
  if (repo == NULL) { return false; }
  if (result_submodule == nullptr) {
    git_submodule *local_submodule;
    int result_code =
        git_submodule_lookup(&local_submodule, repo, path.c_str());
...
advanced_git_repo_source::advanced_git_repo_source(std::string repo_path,
                                                   std::string repo_url)
    : system_git_repo_source(repo_path, repo_url) {}

advanced_git_repo_source::~advanced_git_repo_source() { xmlFreeDoc(xml_doc); }
...
bool advanced_git_repo_source::load_themes_db() {
  ...
  if (!git_pull_specific_path(REPO_MAIN_CONFIG_PATH)) { return false; }
  xml_doc = xmlReadFile((path_to_folder + "/" + REPO_MAIN_CONFIG_PATH).c_str(),
                        NULL, 0);
  themes_db = parse_themes_db_from_xml();
  return !themes_db.empty();
}

std::string advanced_git_repo_source::get_theme_path_for_alias(
    std::string alias) {
  if (themes_db.count(alias) == 0) { return ""; }
  std::cout << "conky: using prests repo " << repo_name << " ["
            << repo_description << "]" << std::endl;
  std::cout << "conky: using theme preset [" << alias << "]" << std::endl;
  std::cout << "conky: pulling theme..." << std::endl;
  if (!git_pull_specific_path(alias + REPO_THEME_FOLDER_POSTFIX)) { return ""; }
...
bool advanced_git_repo_source::set_repo_info_from_info_node(xmlNode *node) {
  ...
  xmlChar *name_value =
      xmlGetProp(node, (const xmlChar *)XML_REPO_INFO_NAME_PROP_NAME);
  if (name_value == NULL) { return false; }
  std::string local_repo_name = (const char *)name_value;
  if (!local_repo_name.empty()) { repo_name = local_repo_name; }
  xmlFree(name_value);

  xmlChar *description_value = xmlNodeGetContent(node);
  if (description_value == NULL) { return false; }
...
bool add_new_theme_from_theme_node_to_map(
    std::map<std::string, std::string> &add_to, xmlNode *node) {
  if (std::string((const char *)node->name) != XML_REPO_THEME_NODE_NAME) {
    return false;
  }
  xmlChar *alias_value =
      xmlGetProp(node, (const xmlChar *)XML_REPO_THEME_ALIAS_PROP_NAME);
  if (alias_value == NULL) { return false; }
...
std::map<std::string, std::string>
advanced_git_repo_source::parse_themes_db_from_xml() {
  std::map<std::string, std::string> result;
  if (xml_doc == NULL) {
    std::cout << "advanced_git_repo_source::cant load xml main repo config file"
              << std::endl;
    return result;
  }
...
bool advanced_git_repo_source::git_init_with_remote_origin(
    std::string path_to_repo, std::string remote_origin_url) {
  if (git_repository_init(&repo, path_to_repo.c_str(), false) != 0) {
    show_last_git_error("advanced_git_repo_source: cant init repo");
    return false;
  }
  git_remote *remote;
  if (git_remote_create(&remote, repo, "origin", remote_origin_url.c_str()) !=
      0) {
...
bool advanced_git_repo_source::git_pull_specific_path(std::string path,
                                                      bool use_base_branch) {
  if (!fetch_repo()) { return false; }
  git_object *target;
  std::string branch_name = BASE_REPO_BRANCH;
  if (!use_base_branch) { branch_name = get_head_branch_name_for_repo(repo); }
  if (git_revparse_single(
          &target, repo, (std::string("origin/") + branch_name).c_str()) != 0) {
