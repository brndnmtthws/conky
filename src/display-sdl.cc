...
#else
conky::disabled_display_output sdl_output_disabled("sdl", "BUILD_SDL");
#endif
...
bool display_output_sdl::main_loop_wait(double t) {
  ...
  if (need_to_update != 0) {
    ...
    if (/*(fixed_size == 0) &&*/
        (text_width + 2 * border_total != surface->w ||
         text_height + 2 * border_total != surface->h)) {
      int width = text_width + 2 * border_total;
      int height = text_height + 2 * border_total;
      int flags = SDL_SWSURFACE | SDL_RESIZABLE;
      int bpp = 32;
      printf("resize %dx%d\n", width, height);
...
void display_output_sdl::clear_text(int exposures) {
  if (surface != nullptr) {
    ...
    SDL_Rect r = {(Sint16)(text_start_x - border_total),
                  (Sint16)(text_start_y - border_total),
